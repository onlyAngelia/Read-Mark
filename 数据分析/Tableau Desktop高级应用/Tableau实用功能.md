...
$操作应用方式\begin{cases} 本地调整本地\\ 本地跳转网页\\ 带筛选条件或不带筛选条件的跳转\end{cases}$

# 9.参数功能

参数分类：嵌入代码的标记参数、创建报表是使用的参数、初始化SQL参数

$参数功能\begin{cases} TopN参数筛选 \\ 更换不同的维度或度量 \\ 控制日期范围 \\ 实现模糊匹配 \\ 控制度量按正序、倒序、默认顺序排序\\ 结合集实现同现现象分析\\切换不同图表\\ 创建本地数据空提取\\ 选择某个数据集中的数据桶 \end{cases}$

# 10. Tableau中常用的显示技巧

$Tableau显示技巧\begin{cases} 处理行列超过默认上限\\ 隐藏坐标轴的刻度、数值值显示标题 \\ 去掉文本表的占位符"Abc"\\ 为没有标签的字段添加标签\\ 为文本表中的标记列添加形状\\ 为上升或下降的数值匹配箭头字符\\ 为上升或下降的数值匹配预警色 \\ 互换图表顶部与底部标签的显示位置\end{cases}$